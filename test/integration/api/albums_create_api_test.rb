...
class Api::AlbumsCreateApiTest < ActionDispatch::IntegrationTest
  ...
  test 'パラメータにnameが足らないときは400を返す' do
    post api_albums_path, headers: { 'Authorization': "Bearer #{ENV.fetch('KITCHOTIFY_CREATE_API_TOKEN')}" }, params: {kiki_taikai_date: 2099-01-01}
    assert_response 400
    res = JSON.parse(response.body)
  ...
  test 'パラメータにkiki_taikai_dateが足らないときは400を返す' do
    post api_albums_path, headers: { 'Authorization': "Bearer #{ENV.fetch('KITCHOTIFY_CREATE_API_TOKEN')}" }, params: {name: 'new_album'}
    assert_response 400
    res = JSON.parse(response.body)
  ...
  test '同名のアルバムが存在している場合409を返す' do
    post api_albums_path, headers: { 'Authorization': "Bearer #{ENV.fetch('KITCHOTIFY_CREATE_API_TOKEN')}" }, params: {name: @album.name, kiki_taikai_date: 2099-01-01}
    assert_response 409
    res = JSON.parse(response.body)
  ...
  test '成功したときは200を返す' do
    assert_difference 'Album.count', 1 do
      post api_albums_path, headers: { 'Authorization': "Bearer #{ENV.fetch('KITCHOTIFY_CREATE_API_TOKEN')}" }, params: {name: 'new_album', kiki_taikai_date: 2099-01-01}
    end
    assert_response 200
